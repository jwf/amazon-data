...
import { getOrdersByCategory, Order } from '../api';

interface CategoryOrderTableProps {
  category: string;
  onClose: () => void;
}

type SortColumn = 'order_date' | 'product_name' | 'total_owed' | 'quantity' | 'order_id';
type SortOrder = 'asc' | 'desc';

const CategoryOrderTable: React.FC<CategoryOrderTableProps> = ({ category, onClose }) => {
  ...
  const loadOrders = async () => {
    setLoading(true);
    try {
      const result = await getOrdersByCategory(
        category,
        ...
        <div className="relative bg-white rounded-lg shadow-xl w-full max-w-7xl max-h-[95vh] flex flex-col">
          ...
          <div className="flex justify-between items-center p-6 border-b border-gray-200 flex-shrink-0">
            <h3 className="text-2xl font-bold text-gray-900">
              Orders for: {category} ({total.toLocaleString()} orders)
            </h3>
            <button
              onClick={onClose}
...
export default CategoryOrderTable;
