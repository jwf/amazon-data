...
const DigitalBreakdownComponent: React.FC = () => {
  const { data, loading, error } = useApiData<DigitalBreakdown>(getDigitalBreakdown);
  const [selectedCategory, setSelectedCategory] = useState<string | null>(null);

  const categoryTotal = useMemo(
    () => data?.categories.reduce((sum, c) => sum + c.spending, 0) ?? 0,
    [data?.categories]
  );
  ...
  if (error) {
    return <div className="text-center py-8 text-red-500">Failed to load digital breakdown</div>;
  }
        ...
        <div className="bg-white rounded-lg shadow p-6">
          ...
          <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-3 max-h-64 overflow-y-auto">
            {data.categories.map((cat, index) => {
              const percent = categoryTotal > 0 ? (cat.spending / categoryTotal) * 100 : 0;
              return (
                <button
                  key={cat.name}
                  onClick={() => setSelectedCategory(cat.name)}
                  className="flex justify-between items-center bg-gray-50 p-2 rounded text-xs cursor-pointer hover:bg-blue-50 hover:border-2 hover:border-blue-300 transition-all text-left border-2 border-transparent"
                >
